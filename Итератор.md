...
## `std::inserter` — краткий конспект

### Что это

`std::inserter(container, pos)`  
— это **output-iterator адаптер**, который при присваивании делает:

```cpp
container.insert(pos, value);
```

Используется, чтобы алгоритмы STL могли вставлять элементы в контейнер.

---

## Зачем нужен

Алгоритмы STL не знают, как добавлять элементы в контейнер.  
Они умеют только:
...
А `insert_iterator` превращает это в:
...
- Это позиция-подсказка (hint).
    
- Для `std::set` почти не влияет на сложность.
    
- Просто стандартный нейтральный вариант.
...
`std::inserter` — это мост между алгоритмами STL и контейнерами с `insert()`.